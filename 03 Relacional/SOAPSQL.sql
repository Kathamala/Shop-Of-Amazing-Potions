...
UPDATE JOGADOR SET dinheiro = dinheiro + 100
WHERE id = 1

SELECT * FROM NPC_ALERGICO_A_INGREDIENTE
DELETE FROM NPC_ALERGICO_A_INGREDIENTE WHERE TRUE

SELECT * FROM INGREDIENTE_TRATA_CONDICOES
SELECT * FROM NPC WHERE JOGADOR_id = 1
AND id NOT IN (SELECT NPC_id FROM TRANSACAO WHERE JOGADOR_id = 1)

SELECT * FROM TRANSACAO
INSERT INTO TRANSACAO VALUES (1, 1, 3)
DELETE FROM TRANSACAO WHERE NPC_id = 2

SELECT * FROM NPC_ALERGICO_A_INGREDIENTE, INGREDIENTE_COMPOE_POCAO
WHERE NPC_ALERGICO_A_INGREDIENTE.INGREDIENTE_id = INGREDIENTE_COMPOE_POCAO.INGREDIENTE_id
AND NPC_id = 3 AND POCAO_id = 1

SELECT INGREDIENTE_id FROM INGREDIENTE_TRATA_CONDICOES, NPC_ACOMETIDO_POR_CONDICOES
WHERE INGREDIENTE_TRATA_CONDICOES.CONDICOES_id = NPC_ACOMETIDO_POR_CONDICOES.CONDICOES_id
AND NPC_id = 3 AND INGREDIENTE_id NOT IN (SELECT INGREDIENTE_id FROM INGREDIENTE_COMPOE_POCAO 
WHERE POCAO_id = 1)
